...
object AuthController : Controller() {
    ...
    const val M_VERIFY_0 = "尝试获取验证码使用 : 邮箱 = {}, 昵称 = {}"
    const val M_VERIFY_1 = "向用户 '{}' 发送邮件成功: {}"
    const val M_VERIFY_2 = "向用户发送邮件时错误:"
    const val M_REGISTER_0 = "尝试注册新的用户使用 : 邮箱 = {}, 昵称 = {}, 验证码 = {}"
    const val M_REGISTER_1 = "新的用户 '{}' 注册成功"
    const val M_REGISTER_2 = "创建用户昵称 '{}' 的默认玩家 : uuid = {}"
    const val M_REGISTER_3 = "用户验证码 '{}' 和预期不符合, 预期 => '{}'"
    const val M_AUTHENTICATE_0 = "用户尝试 Authenticate 使用 : 邮箱 = {}, 客户端令牌 = {}"
    ...
    suspend fun verify(
        ...
        YggdrasilLog.info(M_VERIFY_0, email, nickname)
        ...
        if (verifyCode == null) {
            ...
            try {
                ...
                YggdrasilLog.info(M_VERIFY_1, email0.value, emailResponse)
            } catch (e: Exception) {
                // 解析模板发送邮件失败，移除验证码缓存并抛出服务器内部异常
                verifyCodeCached.remove(email0.value)
                YggdrasilLog.error(M_VERIFY_2, e)
    ...
    suspend fun authenticate(
        ...
        YggdrasilLog.info(M_AUTHENTICATE_0, email, clientToken)
